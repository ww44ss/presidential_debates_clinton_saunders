...
print(xtable(as.matrix(unique(d_all_f$debate))), type="html", size="small")
...
print(xtable(as.matrix(unique(r_all_f$debate))), type="html", size="small")
...
We'll use the 'mutate()' to add a "sentiment" column to the data.
...
The plot below shows the trend of the overall sentiment of the all debates and all candidates. The "other" category lumps all audience and moderator comments together. Notice that the candidate sentiments generally show much higher variablity than "other," with February 11 2016 standing out particularly.  
...
[Coverage](http://www.cnn.com/2016/02/11/politics/democratic-debate-highlights/) of the February 11, 2016 debate revealed the following comments:   
_Much of the debate lacked the bitterness of earlier forums as Clinton and Sanders laid out differences on policy questions. But the confrontation during the PBS "NewsHour" Democratic debate simulcast on CNN flared into open anger in the final moments._
...
####Use __*filter()*__ to drill into February 11 2016  
Again we can use _{dplyr}_ functions to get the data and then use the transform function in the _{syuzhet}_ package to extract low frequency components.

```{r, echo = 2:5, fig.align="center", fig.height=4, fig.width=6}

candidate_x <- "SANDERS"
debate_x <- "February 11 2016"
single_d_debate <- d_all_f %>% filter(debate == debate_x, name == candidate_x)
filtered.sentiment = get_dct_transform(single_d_debate$afinn_sentiment, x_reverse_len=nrow(single_d_debate), low_pass_size = 6) 

tt <- filtered.sentiment %>% as.data.frame()

ggplot(tt, aes(y = filtered.sentiment, x=seq_along(filtered.sentiment))) +
...
temptable<-xtable(cbind(single_d_debate$text[15:17], single_d_debate$afinn_sentiment[15:17]))
colnames(temptable) <- c("text", "afinn_sentiment")
...
print(single_d_debate$text[41:42])
...
Here, there are a lot more candidates so the density of data is lower, especially in the earlier debates, where there was a plethora of them.
...
####Drilling in on March 3, 2016   
The March 3, 2016 debate was of note because it struck possibly the lowest note of the presidential debates. It was heralded in the press [Donald Trump defends size of his penis](http://www.cnn.com/2016/03/03/politics/donald-trump-small-hands-marco-rubio/) in a remarkalbe exchange with Marco Rubio. 

```{r, echo = FALSE, fig.align="center", fig.height=4, fig.width=6}

candidate_x <- "TRUMP"
debate_x <- "March 3 2016"
single_d_debate <- r_all_f %>% filter(debate == debate_x, name == candidate_x)
filtered.sentiment = get_dct_transform(single_d_debate$afinn_sentiment, x_reverse_len=200, low_pass_size = 6) 
...
###Using _filter()_ to plot a histogram of specific candidate sentiment
...
In this case we can see some definite differences in between the sentiments of different debates. Particularly the Feb 4, 2016 debate. We can dig a little deeper by looking at the text
...
These correlate prety well. 

```{r}

ggplot(d_all_f %>% filter(name == "SANDERS" | name == "CLINTON"), aes(x = afinn_sentiment, y = 2*bing_sentiment)) + geom_jitter(pch=21, width=2, height=2, alpha=.4, fill = "darkred") + geom_smooth(formula = y ~ x) + ggtitle("afinn versus bing sentiment")
